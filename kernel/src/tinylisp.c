...
/* hp: heap pointer, A+hp with hp=0 points to the 
 atom string in cell[]
...
/* Lisp primitives:
   ...
   (or x1 x2 ... xk)   
    x that is not (), otherwise ()
   (and x1 x2 ... xk)  last x if all x are not (), otherwise ()
   (cond (x1 y1)
         (x2 y2)
         ...
         (xk yk))      the 
          yi for which xi evaluates to non-()
...
/* Switch input to terminal keyboard explicitly */
...
/* Custom getchar function to handle switching */
int my_getchar() {
...
int main(int argc, char** argv) {
  ...
  cell = (double*)calloc(sp, sizeof(L));
  I i;
  printf("tinylisp");
  init_input();
  nil = box(NIL, 0);
